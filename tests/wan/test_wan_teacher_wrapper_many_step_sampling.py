...
for index in tqdm(range(len(dataset))):
    ...
    with torch.amp.autocast(
        dtype=torch.float32, device_type=torch.device("cuda:0").type
    ):

        for progress_id, t in enumerate(tqdm(scheduler.timesteps)):
            timestep = t * torch.ones([1, 21], device=device, dtype=torch.float32)

            x0_pred_cond = model(latents, conditional_dict, timestep)

            x0_pred_uncond = model(latents, unconditional_dict, timestep)

            x0_pred = x0_pred_uncond + 6 * (x0_pred_cond - x0_pred_uncond)
        ...
        decoded_video = (
            vae.decode_to_pixel(latents)[0].permute(1, 2, 3, 0).cpu().numpy() * 0.5
            + 0.5
        )

    export_to_video(decoded_video, f"test_wanx_wrapper_{index:04d}.mp4", fps=16)
