...
for timestep in [100, 200, 300, 400, 500, 600, 700, 800, 900]:
    timestep = timestep * torch.ones([1, 21], device="cuda", dtype=torch.float32)

    noisy_latent = (
        scheduler.add_noise(
            video_latent.flatten(0, 1), noise.flatten(0, 1), timestep.flatten(0, 1)
        )
        .unflatten(0, noise.shape[:2])
        .type_as(video_latent)
    )
    ...
    export_to_video(video, f"one_stpe_output_t={timestep[0, 0].item()}.mp4", fps=16)
