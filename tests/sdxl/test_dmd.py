...
dmd_model.generator.model.load_state_dict(
    torch.load(hf_hub_download(repo_name, ckpt_name), map_location="cuda")
)

conditional_dict = dmd_model.text_encoder(text_prompts=["a photo of a cat"] * 4)

unconditional_dict = dmd_model.text_encoder(text_prompts=[""] * 4)

print("Test 1: Backward Simulation")

noise = torch.randn(4, 1, 4, 128, 128, generator=torch.Generator().manual_seed(42)).to(
    "cuda"
)
...
simulated_input = dmd_model._consistency_backward_simulation(
    noise=torch.randn(image_or_video_shape, device="cuda", dtype=torch.float32),
...
images = ((images + 1.0) * 127.5).clamp(0, 255).to(torch.uint8).permute(0, 2, 3, 1)
...
critictrain_latent_images = dmd_model.vae.decode_to_pixel(critictrain_latent).squeeze(1)
...
dmdtrain_clean_latent_images = (
    ((dmdtrain_clean_latent_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

dmdtrain_noisy_latent_images = (
    ((dmdtrain_noisy_latent_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

dmdtrain_pred_real_image_images = (
    ((dmdtrain_pred_real_image_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

dmdtrain_pred_fake_image_images = (
    ((dmdtrain_pred_fake_image_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

critictrain_latent_images = (
    ((critictrain_latent_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

critictrain_noisy_latent_images = (
    ((critictrain_noisy_latent_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)

critictrain_pred_image_images = (
    ((critictrain_pred_image_images + 1.0) * 127.5)
    .clamp(0, 255)
    .to(torch.uint8)
    .permute(0, 2, 3, 1)
)
