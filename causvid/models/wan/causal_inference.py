...
class InferencePipeline(torch.nn.Module):
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache1.append(
                {
                    "k": torch.zeros(
                        [batch_size, 32760, 12, 128], dtype=dtype, device=device
                    ),
                    "v": torch.zeros(
                        [batch_size, 32760, 12, 128], dtype=dtype, device=device
                    ),
    ...
    def inference(
        ...
        num_input_blocks = (
            start_latents.shape[1] // self.num_frame_per_block
        ...
        for block_index in range(num_blocks):
            noisy_input = noise[
                :,
                block_index
                * self.num_frame_per_block : (block_index + 1)
            ...
            if start_latents is not None and block_index < num_input_blocks:
                ...
                current_ref_latents = start_latents[
                    :,
                    block_index
                    * self.num_frame_per_block : (block_index + 1)
                    * self.num_frame_per_block,
